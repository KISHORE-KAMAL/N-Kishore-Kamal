if (!customElements.get("custom-hostpot")) {
  class CustomHotspot extends HTMLElement {
    ...
    async appendProductCardContent(e) {
      ...
      try {
        const res = await fetch(`/products/${productHandle}?view=popup-product-card`);
  ...
  customElements.define("custom-hostpot", CustomHotspot);
}